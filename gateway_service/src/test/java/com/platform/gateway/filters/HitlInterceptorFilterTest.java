...
import com.platform.gateway.services.ComplianceAlertService;
...
class HitlInterceptorFilterTest {
    ...
    @Mock private ComplianceAlertService alertService;
    ...
    void TC_GW_01_highRiskRequestIsInterceptedAndNotForwarded() throws Exception {
        ...
        // Officer dashboards must see the new payload without a page refresh (TC-UI-01)
        verify(streamService).broadcast(saved);
    }
    ...
    void TC_GW_04_clientAbortFlipsStatusToDeliveryTimeoutWarning() throws Exception {
        ...
        // TC-SEC-02: the compliance channel must be notified an agent may retry
        verify(alertService).alertDeliveryTimeout(saved);
    }
