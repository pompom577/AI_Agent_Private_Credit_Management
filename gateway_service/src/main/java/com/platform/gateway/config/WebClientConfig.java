...
public class WebClientConfig {
    ...
    @Value("${gateway.hitl.destination-base-url}")
    private String destinationBaseUrl;

    @Bean
    public WebClient classificationWebClient(WebClient.Builder builder) {
        return buildClient(builder, classificationBaseUrl);
    }

    /**
     * WebClient for Sub-Story 3.2a — fires an approved quarantine payload at its
     * original intended destination. Needs an absolute base URL because the
     * parked {@code endpoint} column only stores the request path (e.g.
     * {@code /api/credit/approve}).
     */
    @Bean
    public WebClient destinationWebClient(WebClient.Builder builder) {
        return buildClient(builder, destinationBaseUrl);
    }
