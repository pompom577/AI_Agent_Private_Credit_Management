...
DROP TRIGGER IF EXISTS block_updates_on_hitl_audit_ledger ON hitl_audit_ledger;
DROP TRIGGER IF EXISTS block_deletes_on_hitl_audit_ledger ON hitl_audit_ledger;

CREATE TRIGGER block_updates_on_hitl_audit_ledger
BEFORE UPDATE ON hitl_audit_ledger
FOR EACH ROW EXECUTE FUNCTION protect_audit_ledger();

CREATE TRIGGER block_deletes_on_hitl_audit_ledger
BEFORE DELETE ON hitl_audit_ledger
FOR EACH ROW EXECUTE FUNCTION protect_audit_ledger();

-- Permission Revocation (TC-OPS-02) — defense-in-depth alongside the triggers
-- above; see infrastructure/init-scripts/07-hitl-audit-ledger.sql for the
-- Postgres table-owner caveat. continue-on-error tolerates this statement
-- failing on deployments where DB_USER isn't literally "platform_admin".
REVOKE UPDATE, DELETE ON hitl_audit_ledger FROM platform_admin;