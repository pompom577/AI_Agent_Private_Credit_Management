...
def upload_file(local_path: str, bucket: str, s3_key: str) -> str:
    """
    Upload a local file to S3 and return the canonical s3:// URL.

    Called after each file is extracted from the deal ZIP so that
    StorageStreamService can stream it back to the frontend by its S3 key.
    """
    log.info("Uploading %s -> s3://%s/%s", local_path, bucket, s3_key)
    try:
        _get_s3_client().upload_file(local_path, bucket, s3_key)
